...
| **Intercept 5** | Pursuit-spec saloon. Four turbo boosts instead of three | V8 |
| **Apex GP** | Single seater. Viciously fast and made of glass — contact ends you | Screaming V10 |
| **Hornet 900** | A motorcycle, barely half a car wide. Takes gaps nothing else can | Hard-revving twin |

Every car can finish every circuit at every difficulty — all 120 combinations
are [enforced by the test suite](tests/test_balance.py), so the slow ones are a
choice rather than a trap.

The Hornet's width is not decoration: at 0.17 road half-widths against a car's
0.32, it fits through gaps in oncoming traffic that nothing else can take.

## Hills

The road carries elevation as well as curvature, and the circuits differ in how
much: Neon City is nearly flat, Alpine Crest and Frost Pass run to gradients of
around 15% with crests steep enough to hide the road beyond them. `HILL_SCALE`
in [`game/track.py`](game/track.py) scales the lot.
...
Twelve race tunes, played at random from a shuffled bag so you hear all of them
before any repeats, with the title shown in the corner as a race starts. Each
one is generated by [`tools/gen_music.py`](tools/gen_music.py) from pulse,
triangle and saw oscillators through a small bass/arpeggio/drum sequencer —
there are no sampled instruments anywhere.

## Tests

```bash
./test.sh                  # everything, about 30 seconds
...
| `rules` | Standing start, lap counting and time bonuses, turbo limits, the off-road penalty, per-car handling, the ambulance siren, traffic running the right way on the right side, the jukebox shuffle, and that every car has its sprites and engine bank |
...
| `balance` | An autopilot races every car on every circuit at every difficulty; all 120 combinations must finish with time in hand, and a sloppy run must still be able to fail |
